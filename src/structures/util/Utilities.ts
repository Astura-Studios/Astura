...
import { CategoryDescriptions, CussWords, ErrorEmbedOptions, UtilityDefaults } from "./Interfaces";
import { Captcha } from "../Captcha";
import { Command } from "../Command";
import { DMChannel, GuildMember, Message, MessageCollector, MessageEmbed, MessageEmbedOptions, Role } from "discord.js";
...
export class Utilities {
    ...
    public async runVerification(member: GuildMember, memberRole: Role): Promise<void> {
        const captcha: Captcha = new Captcha();

        await member.createDM()
            .then(async (dm: DMChannel): Promise<void> => {
                dm.send({
                    embeds: [
                        { 
                            color: this.defaults.embed.color,
                            author: {
                                name: `Welcome to ${member.guild.name} - Redefining Limitations`, 
                                iconURL: member.guild.iconURL({ dynamic: true }) as string
                            },
                            description: `Welcome to **${member.guild.name}**! We're a small, engaging community of passionate and determined developers. We strive to help others reach their potential and exceed their limitations, as well as provide support to those along their programming journey.\n\nYou may be wondering what this is? Well, Astura Studios has implemented a special server member verification process to protect our community against malicious bot accounts. You can learn more in ${this.client.markdown.channelMention("803018588323446885")}.\n\nIn order to verify yourself, enter the code in the captcha below. If you get it correct, you will gain gull access to the server. If you fail, you can always retry by typing \`retry\`.\n\nIf you seem to be having issues, feel free to reach out to us in ${this.client.markdown.channelMention("859549114592395295")}`,
                            image: {
                                url: captcha.dataURL
                            }
                        }
                ...
                const collector: MessageCollector = dm.createMessageCollector({ filter });
                const retryCollector: MessageCollector = dm.createMessageCollector({ filter });

                collector.on("collect", (message: Message) => {
                    if (message.content.toUpperCase() === captcha.value) {
                        message.channel.send(`:white_check_mark: **${message.author}, you have successfully have verified yourself. Enjoy the server!**`);    
                        member.roles.add(memberRole);
                        collector.stop();
                        return retryCollector.stop();
                    } else {
                        message.channel.send(`**${message.author}, you sent the wrong code. Please try again by typing \`retry\`.**`);
                        return collector.stop();
                    };
                });

                retryCollector.on("collect", (message: Message) => {
                        ...
                        dm.send({
                            embeds: [
                                { 
                                    color: this.defaults.embed.color,
                                    author: {
                                        name: `{member.guild.name} - Server Member Verification`, 
                                        iconURL: member.guild.iconURL({ dynamic: true }) as string
                                    },
                                    description: `${this.client.markdown.userMention(message.author.id)}, please enter the text shown in the image below in order to be verified.**`,
                                    image: {
                                        url: captcha.dataURL
                                    }
                                }
