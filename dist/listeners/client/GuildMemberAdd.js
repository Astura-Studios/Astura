...
class GuildMemberAddListener extends Listener_1.Listener {
    constructor() {
    ...
    exec(_client, member) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                ;
                if (member.partial) {
                    member = yield member.fetch();
                    const memberRole = member.guild.roles.cache.get("810989551162097696");
                    if (member.user.bot)
                        return;
                    if (member.roles.cache.has(memberRole.id))
                        return;
                    if (!member.pending)
                        return this.client.util.runVerification(member, memberRole);
                }
                else {
                    const memberRole = member.guild.roles.cache.get("810989551162097696");
                    if (member.user.bot)
                ...
                return console.log(`${this.client.util.date.getLocalTime()} | [ Guild Member Add Listener ] ${error.stack}`);
            }
            ;
        });
    }
