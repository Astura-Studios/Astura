        ...
        try {
            ...
            categories.forEach((category) =>
                __awaiter(this, void 0, void 0, function* () {
                    const categoryName =
                        this.client.util.string.capitalize(category);
                    this.categories.set(
                        categoryName,
                        new Category_1.Category(categoryName, {
                            content: null,
                            description: "",
                            type: "listener",
                        })
            ...
            for (const category of categories.values()) {
                for (const listenerFileName of (0, fs_1.readdirSync)(
                    `${this.directory}/${category}`
                ).filter((fileName) => fileName.endsWith(".js"))) {
                    const listenerFile =
                        require(`${this.directory}/${category}/${listenerFileName}`).default;
                ...
                const categoryName =
                    this.client.util.string.capitalize(category);
                const categoryListeners = this.listeners.filter(
                    (listener) =>
                ...
                this.categories.set(
                    ...
                    new Category_1.Category(categoryName, {
                        ...
                        description:
                            this.client.util.categoryDescriptions.listeners[
                                category.toLowerCase()
                            ],
                        type: "listener",
                    })
            ...
            return console.log(
                `${this.client.util.date.getLocalTime()} | [ Listener Handler ] Loaded ${
                    (0, fs_1.readdirSync)(this.directory).length
                } listener event(s)`
            );
        } catch (error) {
            return console.log(
                `${this.client.util.date.getLocalTime()} | [ Listener Handler ] ${
                    error.stack
                }`
            );
        }
