...
class Utilities {
    ...
    errorEmbed(options) {
        ...
        switch (options.type) {
            case "INVALID_ARGS":
                return errorEmbed.invalidArguments(
                    options.client,
                    options.command,
                    options.message,
            ...
            case "MISSING_ARGS":
                return errorEmbed.missingAruments(
                    options.client,
                    options.command,
                    options.message,
                    options.errorMessage
            ...
            case "CLIENT_PERMISSIONS_MISSING":
                return errorEmbed.clientPermissionsMissing(
                    options.client,
                    options.errorMessage,
                    options.message
                );
            case "USER_PERMISSIONS_MISSING":
                return errorEmbed.userPermissionsMissing(
                    options.client,
                    options.errorMessage,
                    options.message
                );
            case "OWNER_ONLY":
                return errorEmbed.ownerOnly(
                    options.client,
                    options.errorMessage,
                    options.message
                );
